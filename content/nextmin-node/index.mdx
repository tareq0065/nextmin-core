...
- Search: **Advanced Query Engine** with `$nestedSearch` and prefix/postfix operators
- Cleanup: **Database Cleanup Utility** to remove unused fields/columns
- Events: **Universal Event System** for global and model-specific hooks
- Schemas: hot‑reload via `SchemaLoader` and automatic model wiring
- Real-time: **Live updates** via `RealtimeService` (Socket.io)
...
async function start() {
  ...
  events.on(Events.AFTER_CREATE, ({ modelName, result }) => {
    console.log(`Document created in ${modelName}:`, result);
  });
