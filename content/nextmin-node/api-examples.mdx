...
- `page`: number (default 1 - **Note: Transitioned to 1-based pagination**)
- `limit`: number (default 12)
- `select` or `fields`: CSV of field names to return (Projection)
- Search helpers:
  ...
  - `searchType`: starts|includes (defaults to starts)
- Date range:
  - `dateFrom`: ISO or parseable date
  - `dateTo`: ISO or parseable date
  - `dateKey`: which date field to use
- Sorting:
  - `sort`: CSV of field names (e.g. `createdAt,name`). Supports prefixes: `+` for ASC, `-` for DESC (e.g. `-createdAt`).
  - `sortType`: CSV of directions (asc|desc), matched by position if no prefix is used in `sort`.
- Advanced Filtering (`where`):
  - Pass a JSON object to the `where` parameter for complex queries.
  - `$nestedSearch`: Search across referenced collections. e.g. `where={"speciality":{"$nestedSearch":"Cardio"}}`
- Field-specific Operators:
  - `!`: Not equal. e.g. `status=!active`
  - `gt:`, `gte:`, `lt:`, `lte:`: Numeric/Date comparisons. e.g. `price=gt:100`
  - `,`: CSV for `$in` queries. e.g. `category=news,updates`
  - `^`: Prefix regex search. e.g. `name=^Alice`
  - `$`: Postfix regex search. e.g. `name=Smith$`

Example:

```bash
curl "http://localhost:8081/rest/posts?limit=20&page=1&sort=-createdAt&select=title,createdAt&q=hello&searchKeys=title,content" \
  -H "x-api-key: $API_KEY" \
  -H "Authorization: Bearer $JWT"
```
...
```json
{
  ...
  "data": [ { "id": "...", "title": "Hello", "createdAt": "..." } ],
  "pagination": { "totalRows": 1, "page": 1, "limit": 20 },
  "sort": { "createdAt": -1 }
}
```
...
`nextmin-node` includes a central event system based on `EventEmitter`. It allows you to hook into CRUD and system operations directly on the backend. This is the foundation for our real-time synchronization and allows you to build custom business logic (e.g., sending emails or triggering external APIs) tied to your data lifecycle.

### Universal Events
You can subscribe to global events using the `Events` constants:

```ts
import { events, Events } from '@airoom/nextmin-node';

events.on(Events.AFTER_CREATE, (payload) => {
  console.log(`[EVENT] New ${payload.modelName} created:`, payload.result);
});
```

Common event constants:
- `Events.BEFORE_CREATE`, `Events.AFTER_CREATE`
- `Events.BEFORE_UPDATE`, `Events.AFTER_UPDATE`
- `Events.BEFORE_DELETE`, `Events.AFTER_DELETE`
- `Events.SCHEMA_UPDATE` (triggered on schema hot‑reloading)
- `Events.SERVER_START`

### Internal Model-Specific Listeners
Starting in <Version />, you can also subscribe to **granular, model-specific** events for more precise handling. These follow the format `modelName:phase:action`.

```ts
// Triggers ONLY when a new Post is created
events.on('posts:after:create', (payload) => {
  console.log(`A new post titled "${payload.result.title}" was created!`);
});

// Triggers ONLY when a user profile is updated
events.on('users:after:update', (payload) => {
  // payload includes: { modelName, id, data, result }
});
```

This granular approach keeps your event-driven architecture clean and performant by avoiding giant `if(modelName === ...)` blocks.
...
async function listPosts(jwt: string) {
  ...
  url.searchParams.set('select', 'title,createdAt');
