...
```jsonc
{
  ...
  "searchType": "includes",             // optional; "starts" (default) or "includes" (substring %term% match)
...
### Nested Search (`$nestedSearch`)

The `NMAdapter` and `MongoAdapter` support searching across referenced collections using the `$nestedSearch` operator in standard CRUD list queries.
...
This efficiently queries the referenced collection (e.g., `Specialities`) and filters the main list based on matching related IDs.

### Search Operators (`^` and `$`)

You can refine your text searches using prefix and postfix operators:

- `^Term`: Matches values that **start with** "Term".
- `Term$`: Matches values that **end with** "Term".
- `Term`: Standard substring match (anywhere in the string).

### Automatic Global Search

The global `q` parameter has been enhanced to automatically include `array`, `json`, and `object` fields. This ensures that data stored in complex structures is searchable without manual configuration.
